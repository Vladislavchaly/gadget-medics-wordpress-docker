...
if ( ! class_exists( 'WPSEO_Local_Core' ) ) {
	...
	class WPSEO_Local_Core {
		...
		public function __construct() {
			...
			add_action( 'init', array( $this, 'enforce_company_settings' ), 14 );
		}
		...
		public static function set_defaults() {
			...
			$defaults = array(
				...
				'api_key_browser'                     => '',
				'api_key'                             => '',
				'enhanced_search'                     => ( empty( $options ) ? 'on' : '' ),
			);
		 ...
		 *
		 * @return bool|array Returns coordinates in array ( Format: array( 'lat', 'long' ) ). False when call the Maps API did not succeed or no address is available.
		 */
		public function get_geo_data( $location_info, $force_update = false, $post_id = 0 ) {
			if ( ! isset( $location_info['_wpseo_business_address'] ) || '' === $location_info['_wpseo_business_address'] ) {
				return false;
			}
			...
			$format       = new WPSEO_Local_Address_Format();
			$full_address = $format->get_address_format( $address_format, $address_details );
		...
		public function check_custom_marker_size( $image_id ) {
			...
			if ( $image[1] > 100 || $image[2] > 100 ) {
				echo '<p class="desc label" style="border:none; margin-bottom: 0;">' . esc_html__( 'The uploaded custom marker exceeds the recommended size of 100x100 px. Please be aware this might influence the info popup.', 'yoast-local-seo' ) . '</p>';
			}
		}
