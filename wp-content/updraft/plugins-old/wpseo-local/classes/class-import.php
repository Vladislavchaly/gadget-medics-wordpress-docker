								...
								if ( $geo_data->get_error_code() === 'wpseo-query-limit' ) {
									$this->messages[] = array(
										...
										'content' => sprintf(
											...
											__( 'The usage of the Google Maps API has exceeded their limits. Please consider entering an API key in the %1$soptions%2$s', 'yoast-local-seo' ),
											'<a href="' . admin_url( 'admin.php?page=wpseo_local' ) . '">',
											'</a>'
										),
									);
									...
									if ( ! empty( $last_imported ) ) {
										$this->messages[] = array(
											...
											'content' => sprintf(
												...
												__( 'The last successfully imported location is <a href="%1$s">%2$s</a>', 'yoast-local-seo' ),
												get_edit_post_link( $last_imported ),
									...
									$this->messages[] = array(
										...
										'content' => sprintf(
											...
											__( 'Location %1$s could not be geo-coded. %2$sEdit this location%3$s.', 'yoast-local-seo' ),
											'<em>' . esc_attr( $location['name'] ) . '</em>',
											'<a href="' . admin_url( 'post.php?post=' . esc_attr( $post_id ) . '&action=edit' ) . '">',
											'</a>'
										),
									);
								}
