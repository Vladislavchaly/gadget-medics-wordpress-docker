...
function wpseo_local_show_map( $atts ) {
	...
	if ( $location_array_str != '' ) {
		...
		// Define whether the map has multiple location or not. If so, the route option should not be shown.
		$has_multiple_location = wpseo_has_multiple_locations();
		if ( true === $has_multiple_location && ( $atts['id'] === 'all' || strpos( $atts['id'], ',' ) !== false ) ) {
			$has_multiple_location = true;
		}

		if ( $atts['show_route'] && false === $has_multiple_location ) {
...
function wpseo_enqueue_geocoder() {
	...
	if ( $wpseo_enqueue_geocoder ) {
		...
		if ( ! empty( $api_key_browser ) ) {
			$query_args['key'] = $api_key_browser;
		}
