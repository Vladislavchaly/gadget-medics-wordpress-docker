	...
	protected function compileButtons($data)
	{
		...
		if ($prev) {
			if ($prev['type'] == 'default') {
                ...
                $btnPrev = "<button tabindex='".$tabIndex."' type='button' data-action='prev' class='ff-btn ff-btn-secondary'>".$prev['text']."</button>";
			} else {
		...
		if ($next) {
			if ($next['type'] == 'default') {
                $tabIndex = \FluentForm\App\Helpers\Helper::getNextTabIndex();
                $btnNext = "<button tabindex='".$tabIndex."' type='button' data-action='next' class='ff-float-right ff-btn ff-btn-secondary'>".$next['text']."</button>";
			} else {
