		...
		if (!get_post_meta(get_the_ID(),'_seopress_pro_rich_snippets_disable_all',true)) {
				...
				if (!empty($post_meta_key)) {
					foreach ($post_meta_key as $key => $value) {
						...
						if ($key =='opening_hours') {
							if (!empty($seopress_pro_schemas[0][$id]['rich_snippets_'.$schema_name][$key]) && function_exists('seopress_if_key_exists') && seopress_if_key_exists($seopress_pro_schemas[0][$id]['rich_snippets_'.$schema_name][$key], 'open')===true) {
						...
						if (!empty($_post_meta_value) && count($_post_meta_value)===7) {
							$_post_meta_value = $_post_meta_value;
						} elseif ($post_meta_value == 'manual_single' || $post_meta_value == 'manual_img_single' || $post_meta_value == 'manual_date_single' || $post_meta_value == 'manual_time_single' || $post_meta_value =='manual_rating_single' || $post_meta_value =='manual_custom_single') {
			...
			function seopress_automatic_rich_snippets_articles_option($schema_datas) {
				...
				if(count(array_filter($schema_datas,'strlen')) != 0) {
					...
					$html .= '{
							...
							if ($article_coverage_start_date && $article_coverage_start_time && $article_type=='LiveBlogPosting') {
								$html .= '"coverageStartTime": "'.$article_coverage_start_date.'T'.$article_coverage_start_time.'",';
							}

							if ($article_coverage_end_date && $article_coverage_end_time && $article_type=='LiveBlogPosting') {
								$html .= '"coverageEndTime": "'.$article_coverage_end_date.'T'.$article_coverage_end_time.'",';
							}

							if ($article_type =='ReviewNewsArticle') {
			...
			function seopress_automatic_rich_snippets_events_option($schema_datas) {
				...
				if(count(array_filter($schema_datas,'strlen')) != 0) {
					...
					$html .= '{
							...
							if ($events_status !='' && $events_status !='none') {
								$html .= '"eventStatus": '.json_encode($events_status).',';
							}
							if ($event_attendance_mode !='' && $event_attendance_mode !='none') {
								if (
									($event_attendance_mode =='OnlineEventAttendanceMode' && $events_location_url !='')
									||
									($event_attendance_mode =='MixedEventAttendanceMode' && $events_location_url !='')
								) {
									$html .= '"eventAttendanceMode": '.json_encode($event_attendance_mode).',';
								}
							}
							if ($events_location_name !='' && $events_location_address !='') {
								if ($event_attendance_mode =='OnlineEventAttendanceMode' && $events_location_url !='') {
									$html .= '"location": {
										"@type":"VirtualLocation",
										"url": '.json_encode($events_location_url).'
									},';
								} elseif ($event_attendance_mode =='MixedEventAttendanceMode' && $events_location_url !='') {
									$html .= '"location": [{
										"@type":"VirtualLocation",
										"url": '.json_encode($events_location_url).'
									},
			...
			function seopress_automatic_rich_snippets_products_option($schema_datas) {
				...
				if(count(array_filter($schema_datas,'strlen')) != 0) {
					...
					if (isset($product) && $products_global_ids =='' && method_exists($product, 'get_id') && get_post_meta($product->get_id(), 'sp_wc_barcode_type_field', true) !='' && get_post_meta($product->get_id(), 'sp_wc_barcode_type_field', true) !='none') {
					...
					if (isset($product) && $products_global_ids_value =='' && method_exists($product, 'get_id') && get_post_meta($product->get_id(), 'sp_wc_barcode_field', true) !='' && get_post_meta($product->get_id(), 'sp_wc_barcode_field', true) !='none') {
			...
			/*stock*/
			$get_stock ='';
			if (isset($product) && method_exists($product, 'managing_stock') && $product->managing_stock() === true) { //if managing stock
				if (method_exists($product, 'is_in_stock') && $product->is_in_stock() === true) {
					$get_stock = seopress_check_ssl().'schema.org/InStock';
				} else { //OutOfStock
					$get_stock = seopress_check_ssl().'schema.org/OutOfStock';
				}
			} elseif (isset($product) && method_exists($product, 'managing_stock') && $product->managing_stock() === false && method_exists($product, 'get_stock_status') && $product->get_stock_status()) {
