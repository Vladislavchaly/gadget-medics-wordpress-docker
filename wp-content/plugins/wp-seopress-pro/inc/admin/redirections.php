...
if (seopress_get_toggle_option('404') =='1') {
	...
	if ( is_admin() ) {
		...
		function seopress_404_maybe_render_blank_state( $which ) {
			...
			if ( $post_type === 'seopress_404' && 'bottom' === $which ) {
		...
		function seopress_404_row_actions($actions, $post) {
			if(get_post_type() === 'seopress_404'){
				...
				if (get_post_meta(get_the_ID(), "_seopress_redirections_enabled", true) =='yes') {
		...
		function seopress_404_count_display_column($column, $post_id) {
			...
			if ($column == 'seopress_404_redirect_date_request') {
				global $wp_version;
			...
			if ($column == 'seopress_404_redirect_ip') {
				...
				if (get_post_status($seopress_get_page->ID) =='publish') {
					if (get_post_meta($seopress_get_page->ID,'_seopress_redirections_enabled',true)) {
						...
						elseif (get_post_meta($seopress_get_page->ID,'_seopress_redirections_value',true)) {
							...
							if ( defined( 'ICL_SITEPRESS_VERSION' ) && $query_param =='with_ignored_param' && isset($get_current_url2['query']) && !empty($get_current_url2['query'] && $page_uri4)) {
								$seopress_redirections_value = html_entity_decode($seopress_redirections_value.'?'.$get_current_url2['query']);
								
							} elseif ($query_param =='with_ignored_param' && isset($get_current_url['query']) && !empty($get_current_url['query'])) {
													...
													<table border="0" cellpadding="10" cellspacing="0" width="100%">
														<tr>
															<td colspan="2" id="credit" style="border:0;color: #878787; border-top: 1px solid #F1F1F1;" valign="middle">
																<p><a href="https://www.seopress.org/">SEOPress</a></p>
		...
		if (is_404() && !is_admin() && seopress_404_redirect_home_option() !='') {
			if (seopress_404_redirect_home_option() =='home') {
				if (seopress_404_redirect_status_code_option() !='') {
					if (seopress_is_bot() !='1' && seopress_404_enable_option()) {
